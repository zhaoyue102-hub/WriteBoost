/**
 * Cloudflare Worker proxy for WriteBoost
 *
 * Endpoints:
 * - GET  /api/power?word=walk
 *   -> proxies to Datamuse "ml" search, returns JSON { levels: {1..5: string[]}, raw: [...] }
 *
 * - POST /api/check
 *   Body: { text: string, language?: string } (JSON)
 *   -> proxies to LanguageTool, returns JSON { matches: [...] } (pass-through)
 *
 * - GET/POST /api/state?code=123456
 *   -> simple cloud sync storage (requires KV binding SYNC_KV)
 *
 * CORS:
 * - Allows any origin by default (kid-friendly simple hosting).
 */
